<template>
	<Error v-if="errorText.length > 0" :text="errorText" v-on="errorBtn ? { btn: fetchReplay } : {}" />
	<Loader v-else-if="loading" />
	<div v-else-if="replay !== null" class="grad-replay">
		<ReplayMap :satShown="satShown" :gridShown="gridShown" :worldName="replay.worldName" :frame="frame" />
		<div class="grad-replay__top">
			<ReplayTitle :replay="replay" />
			<ReplayToolbar :satShown.sync="satShown" :gridShown.sync="gridShown" />
		</div>
		<ReplayControls v-model="frame" :replay="replay" />
	</div>
</template>

<script lang="ts">
import { Component, Vue, Prop } from 'vue-property-decorator';
import ErrorVue from '@/components/Replay/Error.vue';
import LoaderVue from '@/components/Replay/Loader.vue';
...
export default class MissionAfterActionReport extends Vue {
