<template>
	<Error v-if="errorText.length > 0" :text="errorText" @btn="fetchReplays" />
	<Loader v-else-if="loading" />
	<div v-else class="grad-replays">
		<ReplaysFilter v-model="filteredReplays" :replays="replays" :maps="maps" />
		<div class="grad-replays__replays-wrapper" v-if="filteredReplays.length > 0">
			<ReplayItem v-for="r in filteredReplays" :key="r.id" :model="r" :maps="maps" />
		</div>
...
export default class MissionAfterActionReportOverview extends Vue {
