			...
			<template v-for="(navlink, index) in navigationLink.links">
				<template v-if="navlink && navlink.tooltip">
					<v-tooltip bottom :key="index">
						<template v-slot:activator="{ on }">
							<v-btn min-width="0" text v-on="on" :to="navlink.to" class="ml-4">
				...
				<template v-if="navlink && navlink.sublinks && navlink.sublinks.length > 0">
					<v-menu
						...
						offset-y
						:key="index"
						v-if="$tstore.state.auth.roles.some(role => navlink.roles.indexOf(role) >= 0)"
					>
						<template v-slot:activator="{ on }">
						...
						<v-list dense>
							<v-list-item-group>
								<template v-for="(sublink, sublinkIndex) in navlink.sublinks">
									<v-list-item :key="sublinkIndex" :to="sublink.to">
										<v-list-item-icon>
						...
						<template v-if="navlink && navlink.sublinks && navlink.sublinks.length > 0">
							...
							>
								<v-list-group no-action value="true" class="text-wrap" color="info">
									...
									<v-list-item
										v-for="(sublink, sublinkIndex) in navlink.sublinks"
										:key="sublinkIndex"
										:to="sublink.to"
										link
									>
