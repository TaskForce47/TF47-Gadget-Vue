<template>
	<div class="grad-replay__map">
		<Error v-if="errorText.length > 0" :text="errorText" v-on="errorBtn ? { btn: loadMap } : {}" />
...
import UnitMarker from '@/services/utils/UnitMarker';
...
export default class ReplayMapVue extends Vue {
	...
	private async loadMap() {
		...
		try {
			this.map = await GradMap.new(this.worldName, this.$refs.map as HTMLDivElement, {
