...
import DataWorker from '@/services/utils/DataWorker';
...
export default class ReplayControlsVue extends Vue {
	...
	// v-model helpers
	private get frame(): ReplayFrame | null {
		return this.value;
	}
	private set frame(val: ReplayFrame | null) {
		this.$emit('input', val);
	}
	...
	private created() {
		...
		this.dataWorker.addEventListener('loading', () => {
			this.loading = true;
		});
