...
export default class Playeruidsetter extends Vue {
  ...
  private rules: InputValidationRules = [
    ...
    value =>
      (value && value.length == 17) || "The Player Uid is 17 characters long",
    value =>
      (value && !isNaN(value)) || "The Player Uid only consist of numbers"
  ];
  private formValid = false;
  private requestInProgress = false;
  private setPlayerUid(playerUid: number) {
    this.requestInProgress = true;
    setUserPlayerUid({ playerUid: playerUid.toString() }).then(
      (response: boolean) => {
        if (response) {
